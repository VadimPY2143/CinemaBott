...
with open('films (3).json', 'r', encoding="utf-8") as file:
...
async def set_default_commands(dp):
    await bot.set_my_commands(
        [
            ...
            types.BotCommand('add_films', 'Додати новий фільм.')
        ]
    )
...
async def add_new_film(message: types.Message, state: FSMContext):
    ...
    if user_id in ADMINS:
        await message.answer(text='Введіть назву фільму який ти хочеш додати')
        await state.set_state('set_film_name')
    else:
        await message.answer(text='У вас недостатньо прав для доступу')
